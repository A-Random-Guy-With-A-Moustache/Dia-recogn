...
def init_model():
    ...
    model_path = MODELS_DIR / "analyzer-4B"
...
with gr.Blocks(title="VLM DSL и помощник по вопросам") as demo:
    gr.Markdown("## Интерфейс базовой модели VLM")
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            with gr.Row():
                ...
                btn_copy_dsl = gr.Button("Копировать DSL", variant="secondary")
    ...
    btn_copy_dsl.click(None, inputs=[dsl_box], outputs=None, js="function(dsl){if(dsl&&dsl.trim()){navigator.clipboard.writeText(dsl);return 'Скопировано'}return 'Пусто'}")
